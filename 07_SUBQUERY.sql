...
-- 1) 전 직원의 평균 급여 조회하기
SELECT CEIL(AVG(SALARY)) || '원'
FROM EMPLOYEE; -- 3047663원
...
SELECT E.EMP_ID, E.EMP_NAME, DEPT_TITLE, JOB_NAME
FROM EMPLOYEE e 
JOIN JOB USING(JOB_CODE)
...
SELECT E.EMP_ID, E.EMP_NAME, DEPT_TITLE, JOB_NAME, '사수'
FROM EMPLOYEE e 
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE EMP_ID IN (SELECT DISTINCT E.MANAGER_ID
				FROM EMPLOYEE e
				WHERE MANAGER_ID IS NOT NULL);


-- 4) 일반 직원에 해당하는 사원들 정보 조회 (이때, 구분은 '사원'으로)
SELECT E.EMP_ID, E.EMP_NAME, DEPT_TITLE, JOB_NAME, '사원'
FROM EMPLOYEE e 
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE EMP_ID NOT IN (SELECT DISTINCT E.MANAGER_ID
				FROM EMPLOYEE e
				WHERE MANAGER_ID IS NOT NULL);
...
-- > ANY : 과장급여 3개중, 최소급여인 220 보다 더 큰 SALARY
-- < ANY : 과장급여 3개중, 최대급여인 376 보다 더 작은 SALARY
...
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)							
WHERE (DEPT_ID, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE FROM EMPLOYEE WHERE EMP_NAME = '노옹철')
AND EMP_NAME <> '노옹철';					
